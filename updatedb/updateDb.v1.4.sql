...
CHANGE COLUMN `webSiteTitle` `webSiteTitle` VARCHAR(45) NOT NULL DEFAULT 'AVideo' ,
...
ALTER TABLE `users` 
ADD COLUMN  IF NOT EXISTS `status` ENUM('a', 'i') NOT NULL DEFAULT 'a' AFTER `isAdmin`,
ADD COLUMN IF NOT EXISTS `photoURL` VARCHAR(255) NULL DEFAULT NULL AFTER `status`,
ADD COLUMN IF NOT EXISTS  `lastLogin` DATETIME NULL DEFAULT NULL AFTER `photoURL` ,
ADD COLUMN  IF NOT EXISTS `recoverPass` VARCHAR(255) NULL DEFAULT NULL AFTER `lastLogin`;
