...
<?php
}  elseif (!empty($_REQUEST['tags_id']) && class_exists('Tags') && class_exists('VideoTags')) {
    $tag = new Tags($_REQUEST['tags_id']);
    echo PHP_EOL . "<!-- OpenGraph tags_id {$_REQUEST['tags_id']} -->" . PHP_EOL;
    $imgw = 200;
    $imgh = 200;
    $img = Configuration::getOGImage();
    $title = html2plainText($tag->getName());
    $url = VideoTags::getTagLink($_REQUEST['tags_id']);
    ?>
    <meta property="og:type" content="website" />
    <meta property="og:site_name" content="<?php echo $title; ?>">
<?php
} elseif (!isVideo() && empty($videos_id)) {
    echo PHP_EOL . "<!-- OpenGraph empty video -->" . PHP_EOL;
    $imgw = 200;
    $imgh = 200;
    $img = Configuration::getOGImage();
