...
foreach ($categories as $key => $value) {
    ...
    $categories[$key]['comment'] = '<div class="pull-left"><img src="'.User::getPhoto($value['id']).'" alt="" class="img img-responsive img-circle" style="max-width: 50px;"/></div><div class="commentDetails"><div class="commenterName"><strong>'.$name.'</strong> <small>'.humanTiming(strtotime($value['created'])).'</small></div>'. nl2br($value['comment']).'</div>';
}
