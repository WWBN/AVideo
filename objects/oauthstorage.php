    ...
    OpenIDAuthorizationCodeInterface
{
    ...
    public function checkClientCredentials($client_id, $client_secret = null)
    {
      ...
      $res = \sqlDAL::readSql("SELECT * from ".$this->config['client_table']." WHERE client_id = ?","i",array($client_id));
      $result = \sqlDAL::fetchAssoc($res);
      \sqlDAL::close($res);
        //$stmt =  $global['mysqli']->prepare(sprintf('SELECT * from %s where client_id = :client_id', $this->config['client_table']));
    ...
    {
        $stmt = $this->db->prepare($sql = sprintf('SELECT * from %s where user=:username', $this->config['user_table']));
     ...
     * @return bool
     */
    public function setUser($username, $password, $name = null)
    {
        ...
        if ($this->getUser($username)) {
            $stmt = $this->db->prepare($sql = sprintf('UPDATE %s SET password=:password, name=:name where user=:username', $this->config['user_table']));
        } else {
            $stmt = $this->db->prepare(sprintf('INSERT INTO %s (user, password, name) VALUES (:username, :password, :name)', $this->config['user_table']));
        }

        return $stmt->execute(compact('user', 'password', 'name'));
    }
