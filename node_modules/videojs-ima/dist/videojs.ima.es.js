...
var GOOGLE_DAI_TIMED_METADATA_KEY = 'TXXX';
...
SdkImpl.prototype.onAddTrack = function (event) {
  ...
  if (track.kind === 'metadata') {
    ...
    track.addEventListener('cuechange', function (e) {
      // Check for both `activeCues` and `activeCues_` properties
      // which are available depending on the browser.
      var activeCues = track.activeCues_ || track.activeCues;
      if (!activeCues) {
        return;
      }
      var _iterator = _createForOfIteratorHelper(activeCues),
        _step;
      try {
        for (_iterator.s(); !(_step = _iterator.n()).done;) {
          var cue = _step.value;
          if (cue.value && cue.value.key === GOOGLE_DAI_TIMED_METADATA_KEY) {
            _this2.streamManager.onTimedMetadata(_defineProperty({}, GOOGLE_DAI_TIMED_METADATA_KEY, cue.value.data));
          }
        }
      } catch (err) {
        _iterator.e(err);
