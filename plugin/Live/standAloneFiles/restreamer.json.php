...
function startRestream($m3u8, $restreamsDestinations, $logFile, $tries = 1) {
    ...
    if (count($restreamsDestinations) > 1) {
        ...
        $command = "{$ffmpegBinary} -timeout 15000 +discardcorrupt -i \"{$m3u8}\" ";
        foreach ($restreamsDestinations as $value) {
        ...
        if(!isOpenSSLEnabled() && preg_match("/rtpms:/i", $restreamsDestinations[0])){
            ...
            $command = "{$ffmpegBinary} -timeout 15000 +discardcorrupt -i \"{$m3u8}\" -max_muxing_queue_size 1024 -acodec copy -bsf:a aac_adtstoasc -vcodec copy -preset veryfast -f flv \"{$restreamsDestinations[0]}\"";
        }
    }
