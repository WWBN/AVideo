        ...
        foreach ($list as $value) {
            ...
            if ($value['isLocal'] && $filesize > 20) {
                if($filesize != $value['remote']['remote_filesize']){
                    ...
                    _error_log("CDNStorage::put skip because remote file has the same size {$value['remote']['remote_filesize']} {$value['remote']['relative']}");
                }
            } else {
        ...
        while ($continue) {
            ...
            foreach ($ret as $key => $r) {
                ...
                if ($r == FTP_FINISHED) {
                    ...
                    _error_log("CDNStorage::put:uploadToCDNStorage [$key] [{$fileUploadCount}/{$totalFiles}] FTP_FINISHED in {$seconds} {$humanFilesize} {$mbps}/Mbps ETA: {$ETA}");
