...
if ($oIMDB->isReady) {
    ...
    if (empty($_GET['what']) || $_GET['what'] == 4) {
        ...
        $encoderURL = $config->getEncoderURL() . "youtubeDl.json?webm={$plugin->encodeWebm}&videoURL=" . urlencode($trailer) . "&webSiteRootURL=" . urlencode($global['webSiteRootURL']) . "&user=" . urlencode(User::getUserName()) . "&pass=" . urlencode(User::getUserPass());
