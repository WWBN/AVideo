...
require_once $global['systemRootPath'].'plugin/Meet/api/validateMeet.php';

if (!Meet::validatePassword($meet_schedule_id, @$_POST['meet_password'])) {
    echo "confirm password"; die();
    header("Location: {$global['webSiteRootURL']}plugin/Meet/api/confirmMeetPassword.php?meet_schedule_id={$meet_schedule_id}&domain={$domain_}");
    exit;
}
...
<html>
    <head>
        ...
        <?php
        include $global['systemRootPath'] . 'plugin/Meet/api/api.js.php';
        ?>
    </head>
    <body>
...
?>
            ...
            function _readyToClose() {
                ...
                window.location.replace("<?php echo base64_decode($end_meet_redirect); ?>")
            }
