    ...
    <div class="col-sm-5">
        <div class="panel panel-default" id="roomConfiguration">
            <div class="panel-heading">
                <i class="fa fa-plus"></i> <?php echo __("Create Room"); ?>
            </div>
                ...
                <form id="formMeetManager">
                    ...
                    <div class="form-group col-sm-6">
                        ...
                        <div class="input-group">
                            ...
                            <input id="RoomPasswordNew" name="RoomPasswordNew" type="password"  placeholder="" class="form-control" autocomplete="new-password">
                                <span class="input-group-addon" style="cursor: pointer;" id="toggle_RoomPasswordNew"  data-toggle="tooltip" data-placement="left" title="<?php echo __('Show/Hide Password'); ?>"><i class="fa fa-eye-slash"></i></span>
                        </div>
                        <script>
                            $(document).ready(function () {
                                $('#toggle_RoomPasswordNew').click(function () {
                                    $(this).find('i').toggleClass("fa-eye fa-eye-slash");
                                    if ($(this).find('i').hasClass("fa-eye")) {
                                        $("#RoomPasswordNew").attr("type", "text");
                                    } else {
                                        $("#RoomPasswordNew").attr("type", "password");
                                    }
                                })
                            });
                        </script>
                    ...
                    <div class="form-group col-sm-6">
                        ...
                        <label for="publicNew">Access:</label>
                    ...
                    <div class="form-group col-sm-12">
                        <div class="btn-group justified">
                            <span class="btn btn-success" id="newMeet_scheduleLink" onclick="clearMeetForm(true)"><i class="fa fa-plus"></i> <?php echo __("New"); ?></span>
                            <button class="btn btn-primary" type="submit"><i class="fa fa-save"></i> <?php echo __("Save"); ?></button>
                        </div>
            ...
            <div class="panel-body tabbable-line" id="manageTabs">
                ...
                <div class="tab-content">
                    <div id="mToday" class="tab-pane fade in active" style="padding: 10px;" url="<?php
                            echo $global['webSiteRootURL'] . 'plugin/Meet/api/meet_scheduled.php?meet_scheduled=today&manageMeetings=1&'.$userCredentials;
                            ?>"><div class="loader"></div></div>
                    <div id="mUpcoming" class="tab-pane fade" style="padding: 10px;" url="<?php
                    echo $global['webSiteRootURL'] . 'plugin/Meet/api/meet_scheduled.php?meet_scheduled=upcoming&manageMeetings=1&'.$userCredentials;
                            ?>"><div class="loader"></div></div>
                    <div id="mPast" class="tab-pane fade" style="padding: 10px;" url="<?php
                    echo $global['webSiteRootURL'] . 'plugin/Meet/api/meet_scheduled.php?meet_scheduled=past&manageMeetings=1&'.$userCredentials;
                            ?>"><div class="loader"></div></div>
                </div>
            </div>
        </div>
...
<script>
    ...
    $(document).ready(function () {
                ...
                success: function (response) {
                    if (response.error) {
                        ...
                        if ($("#whenNew").val() == "1") {
                            ...
                            modal.hidePleaseWait();
                            var link = response.link.replace("<?php echo $global['webSiteRootURL'] ?>", "<?php echo $domain ?>/");
        ...
        $('#manageTabs .nav-tabs a').click(function (e) {
            ...
            $("#" + divid).attr('url', '');
            var end_meet_redirect = "<?php echo $end_meet_redirect; ?>";
            var domain = "<?php echo $domain ?>";
            if (url) {
                $.ajax({
                    url: url,
                    data: {domain: domain, end_meet_redirect: end_meet_redirect},
                    xhrFields: {
