...
if (isset($_GET['meet_schedule_id'])) {
    $meet_schedule_id = $_GET['meet_schedule_id'];
}
...
if (Meet::validatePassword($meet_schedule_id, @$_POST['meet_password'])) {
    ...
    $domain = parse_url($global['webSiteRootURL'])['host'];
    $url = str_replace($domain, $_REQUEST['domain'], $url);
    header("Location: {$url}");
    exit;
}
